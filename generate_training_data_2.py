...
if __name__ == "__main__":
    ...
    if args.batch == 1:
        ...
        companies = [
            # Tech — high success rate in batch 1
            "ORCL", "ADBE", "CSCO", "AMD", "QCOM",
            "TXN", "NOW", "AVGO",
            # Consumer/Retail — standard 10-K format, good parse rate
            "NKE", "MCD", "CAT", "HD", "COST",
            # Healthcare — worked well in batch 1
            "ABBV", "LLY", "MRK",
            # Energy/Industrial
            "LMT", "RTX",
            # Media/Telecom
            "NFLX", "T",
            # Removed: C, BLK, AXP, SNOW, NEE, SLB — banks/finance
            # have non-standard 10-K formats that our parser can't handle
        ]
    ...
    # --- Estimate batch 2 output and show projected combined balance ---
    # Batch 1 had: S=93 D=45 R=48
    if args.batch == 2:
        # To balance combined total, batch 2 should produce more diff/red_flag
        # Summary: use only ["1A"] (1 section × 2 years ≈ 2 per company ≈ 40 total)
        # Diff: use all 4 sections (≈ 1 per valid section ≈ 60 total)
        # Red_flag: use all 4 sections (≈ 1 per valid section ≈ 60 total)
        est_s = min(total_summary, 20) * 2  # rough estimate
        est_d = total_diff
        est_r = total_summary
        print(f"\n  Estimated batch 2: S≈{est_s} D≈{est_d} R≈{est_r}")
        print(f"  Projected combined: S≈{93+est_s} D≈{45+est_d} R≈{48+est_r}")

    # Wait for user to review validation before spending API credits
    ...
    for i, company in enumerate(companies):
        ...
        # For batch 2: limit to ["1A"] only to reduce summary count
        # (batch 1 already has 93 summaries, we need fewer in batch 2)
        summary_ok = [s for s, v in sections.items() if v in ("both", "latest")]
        if args.batch == 2:
            summary_ok = [s for s in summary_ok if s == "1A"]
        if summary_ok:
        ...
        # Use all valid sections to maximize diff count
        ...
        # Red flag: use all valid sections
    ...
    print(f"  Batch balance: S:{stats['section_summary']} D:{stats['change_detection']} "
          f"R:{stats['red_flag']}")
    if args.batch == 2:
        # Show projected combined totals (batch 1 had S:93 D:45 R:48)
        cs = 93 + stats['section_summary']
        cd = 45 + stats['change_detection']
        cr = 48 + stats['red_flag']
        ct = cs + cd + cr
        print(f"  Combined balance: S:{cs}({cs*100//ct}%) D:{cd}({cd*100//ct}%) "
              f"R:{cr}({cr*100//ct}%) Total:{ct}")
    print(f"{'=' * 60}")
